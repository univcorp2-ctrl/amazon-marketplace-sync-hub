...
class AmazonSPAPIClient:
    def __init__(self, settings: Settings, client: httpx.AsyncClient | None = None) -> None:
        ...
        self.catalog_limiter = AsyncRateLimiter(0.75)
    ...
    async def _request(self, method: str, path: str, *, params: dict[str, Any] | None = None, json: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        headers = {"x-amz-access-token": token, "x-amz-date": datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ"), "user-agent": "AmazonMarketplaceSyncHub/0.3 (Language=Python/3.12)", "accept": "application/json"}
        ...
        for attempt in range(self.settings.amazon_max_retries + 1):
            ...
            delay = float(retry_after) if retry_after else min(30.0, (2 ** attempt) + random.uniform(0.25, 1.25))
            await asyncio.sleep(max(1.0, min(delay, 30.0)))
        raise RuntimeError("Amazon request retry loop exhausted")

    async def search_asins(self, keywords: str, max_items: int = 10) -> list[str]:
        keywords = keywords.strip()
        if not keywords:
            raise ValueError("Search keywords are required")
        if not self.settings.api_live_enabled or self.settings.app_mode == "demo":
            return ["B0DEMO1234"]
        max_items = max(1, min(max_items, self.settings.amazon_search_page_size * self.settings.amazon_search_max_pages))
        await self.catalog_limiter.wait()
        body = await self._request("GET", "/catalog/2022-04-01/items", params={"marketplaceIds": self.settings.amazon_marketplace_id, "keywords": keywords, "pageSize": min(self.settings.amazon_search_page_size, max_items), "includedData": "summaries,productTypes,classifications", "locale": "ja_JP"})
        items = body.get("items") or body.get("payload", {}).get("items") or []
        return [str(item.get("asin", "")).upper() for item in items if item.get("asin")][:max_items]
