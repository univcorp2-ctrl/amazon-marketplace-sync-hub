...
class AmazonSPAPIClient:
    ...
    async def _request(
        ...
        *,
        ...
        headers = {
            ...
            "user-agent": "AmazonMarketplaceSyncHub/0.1 (Language=Python/3.12)",
            "accept": "application/json",
        }
        response = await self.client.request(
            method,
            f"{self.settings.amazon_sp_api_endpoint.rstrip('/')}{path}",
            params=params,
            json=json,
            headers=headers,
        )
    ...
    def _normalize(
        ...
        image_groups = catalog.get("images") or []
        images = [
            image["link"]
        ...
        positive_prices = [p for p in prices if p[0] > 0]
        price = min((p[0] for p in positive_prices), default=None)
        currency = next((p[1] for p in positive_prices if p[1]), "JPY")
        offer_payload = offers.get("payload", offers)
        available = bool(
            offer_payload.get("Offers")
            or offer_payload.get("Summary")
            or self._contains_key(pricing, "featuredBuyingOptions")
        )
    ...
    def _demo_product(self, asin: str) -> ProductRecord:
        return ProductRecord(
            ...
            identifiers=[
                {
                    "marketplaceId": self.settings.amazon_marketplace_id,
                    "identifiers": [{"identifierType": "ASIN", "identifier": asin}],
                }
            ],
            dimensions=[
                {
