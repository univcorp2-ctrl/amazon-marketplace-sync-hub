...
class ShopeeClient:
    ...
    def _signature(self, path: str, timestamp: int) -> str:
        base = (
            f"{self.settings.shopee_partner_id}{path}{timestamp}"
            f"{self.settings.shopee_access_token}{self.settings.shopee_shop_id}"
        )
        return hmac.new(
            self.settings.shopee_partner_key.encode(), base.encode(), hashlib.sha256
        ).hexdigest()
    ...
    async def _post(
        ...
        *,
        ...
        if files:
            response = await self.client.post(url, params=params, data=payload, files=files)
        else:
    ...
    async def upload_images(self, image_urls: list[str]) -> list[str]:
        ...
        for index, image_url in enumerate(image_urls[:8]):
            ...
            body = await self._post(
                ...
                {},
                files=[("image", (f"image-{index}.jpg", image_response.content, content_type))],
            )
    ...
    async def list_product(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        image_ids = await self.upload_images(payload.pop("image_urls", []))
        payload["image"] = {"image_id_list": image_ids}
    ...
    async def update_price(self, item_id: int | str, price: int) -> dict[str, Any]:
        ...
        return await self._post(
            ...
            {"item_id": int(item_id), "price_list": [{"model_id": 0, "original_price": price}]},
        )
    ...
    async def update_stock(self, item_id: int | str, stock: int) -> dict[str, Any]:
        ...
        return await self._post(
            ...
            {"item_id": int(item_id), "stock_list": [{"model_id": 0, "seller_stock": [{"stock": stock}]}]},
        )
