...
from app.connectors.amazon import AsyncRateLimiter


class ShopeeCircuitOpen(RuntimeError):
    pass
...
class ShopeeClient:
    def __init__(self, settings: Settings, client: httpx.AsyncClient | None = None) -> None:
        ...
        self.limiter = AsyncRateLimiter(settings.shopee_request_interval_seconds)
    ...
    async def _post(self, path: str, payload: dict[str, Any], *, files: list[tuple[str, tuple[str, bytes, str]]] | None = None) -> dict[str, Any]:
        if self._circuit_open:
            raise ShopeeCircuitOpen("Shopee circuit breaker is open")
        async with self._lock:
            for attempt in range(self.settings.shopee_max_retries + 1):
                if self._calls >= self.settings.shopee_max_calls_per_process:
                    self._circuit_open = True
                    raise ShopeeCircuitOpen("Shopee per-process API call budget exhausted")
                ...
                if response.status_code == 403:
                    self._circuit_open = True
                    raise ShopeeCircuitOpen("Shopee returned HTTP 403; automatic calls stopped")
                if response.status_code == 429:
                    self._consecutive_throttles += 1
                    if self._consecutive_throttles >= 3 or attempt >= self.settings.shopee_max_retries:
                        self._circuit_open = True
                        raise ShopeeCircuitOpen("Shopee throttled repeated requests; automatic calls stopped")
                    retry_after = response.headers.get("retry-after")
                    delay = float(retry_after) if retry_after else min(60.0, (2 ** attempt) * 3 + random.uniform(0.5, 2.0))
                    await asyncio.sleep(max(3.0, min(delay, 60.0)))
                    continue
                if response.status_code in {500, 502, 503, 504} and attempt < self.settings.shopee_max_retries:
                    await asyncio.sleep(min(30.0, (2 ** attempt) * 2 + random.uniform(0.5, 1.5)))
                    continue
                ...
                if body.get("error"):
                    ...
                    if "auth" in error.casefold() or "permission" in error.casefold():
                        self._circuit_open = True
                    raise RuntimeError(f"Shopee API error: {error} {body.get('message', '')}")
                return body
        raise RuntimeError("Shopee request retry loop exhausted")

    async def upload_images(self, image_urls: list[str]) -> list[str]:
        image_ids: list[str] = []
        for index, image_url in enumerate(image_urls[: self.settings.shopee_max_images_per_listing]):
    ...
    async def list_product(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not payload["image"]["image_id_list"]:
            raise RuntimeError("Shopee listing requires at least one permitted image")
