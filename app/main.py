...
class Container:
    def __init__(self, settings: Settings) -> None:
        ...
        self.marketplaces = MarketplaceService(settings, self.db, self.catalog, self.shopee, self.qoo10)
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title=selected.app_name, version="0.3.0", lifespan=lifespan)
    app.state.container = container
    app.add_middleware(CORSMiddleware, allow_origins=selected.cors_origins, allow_credentials=False, allow_methods=["*"], allow_headers=["*"])
    ...
    async def require_admin(x_admin_key: str | None = Header(default=None, alias="X-Admin-Key")) -> None:
        ...
        if not selected.api_admin_key:
            raise HTTPException(status_code=503, detail="Production API is unavailable until API_ADMIN_KEY is configured")
        if x_admin_key is None or not secrets.compare_digest(x_admin_key, selected.api_admin_key):
    ...
    async def health() -> dict[str, Any]:
        ...
        return {"status": "ok", "mode": selected.app_mode, "live_api_enabled": selected.api_live_enabled, "ready_for_live_listing": all(readiness.values()), "readiness": readiness, "bulk_listing_enabled": selected.bulk_listing_enabled, "shopee_market": selected.shopee_market or None, "amazon_source": "SP-API", "exact_amazon_stock_quantity": False}
    ...
    @app.post("/api/automation/shopee", dependencies=protected)
